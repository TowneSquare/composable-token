...
module composable_token::composable_token_entry {
    ...
    use composable_token::composable_token::{Self, Collection, Composable, Indexed, Named, Trait};
    ...
    const ETOKEN_NOT_EQUIPPED: u64 = 2;
    ...
    public entry fun create_collection_with_fixed_supply_and_royalty(
        ...
        let constructor_ref = composable_token::create_collection<collection::FixedSupply>(
            signer_ref,
            description,
    ...
    public entry fun create_collection_with_unlimited_supply_and_royalty(
        ...
        let constructor_ref = composable_token::create_collection<collection::UnlimitedSupply>(
            signer_ref,
            description,
            option::none(),
            name,
    ...
    public entry fun create_named_composable_token_with_no_royalty(
        ...
        let constructor_ref = composable_token::create_token<Composable, Named>(
            signer_ref,
            collection,
            description,
            name,
    ...
    public entry fun create_named_trait_token_with_no_royalty(
        ...
        let constructor_ref = composable_token::create_token<Trait, Named>(
            signer_ref,
            collection,
            description,
            name,
    ...
    public entry fun equip_fungible_asset<FA: key, TokenType: key>(
        ...
        composable_token::equip_fa_to_token<FA, TokenType>(signer_ref, fa, token_obj, amount);
    }
    ...
    public entry fun decompose_entire_composable_token(
        ...
        let traits = composable_token::traits_from_composable(composable_obj);
        for (i in 0..vector::length<Object<Trait>>(&traits)) {
            let trait_obj = *vector::borrow(&traits, i);
            composable_token::unequip_trait(owner, composable_obj, trait_obj, new_uri);
            i = i + 1;
    ...
    public entry fun replace_traits_of_composable(
        owner: &signer,
        composable_obj: Object<Composable>,
        traits_to_remove: vector<Object<Trait>>,
        traits_to_add: vector<Object<Trait>>,
        new_uri: String // User should not prompt this! It should be generated by the studio.
    ) {
        for (i in 0..vector::length<Object<Trait>>(&traits_to_remove)) {
            let trait_obj = *vector::borrow(&traits_to_remove, i);
            assert!(object::is_owner<Trait>(trait_obj, object::object_address(&composable_obj)), ETOKEN_NOT_EQUIPPED);
        };
        for (i in 0..vector::length<Object<Trait>>(&traits_to_add)) {
            let trait_obj = *vector::borrow(&traits_to_add, i);
            assert!(object::is_owner<Trait>(trait_obj, signer::address_of(owner)), ENOT_OWNER);
        };

        // unequip all the traits from traits_to_remove
        composable_token::unequip_traits(owner, composable_obj, traits_to_remove, new_uri);

        // equip all the traits from traits_to_add
        composable_token::equip_traits(owner, composable_obj, traits_to_add, new_uri);
    }
    ...
    public entry fun transfer_fungible_asset<FA: key>(
        ...
        composable_token::transfer_fa<FA>(signer_ref, recipient, fa, amount);
    }
